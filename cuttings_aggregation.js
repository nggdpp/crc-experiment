db.getCollection("cuttings_raw").aggregate(
	...
	[
		...
		// Stage 2
		{
			$project: {
			    ...
			    "Min": {"$arrayElemAt": ["$intervals.Min", 0]},
			    "Max": {"$arrayElemAt": ["$intervals.Max", 0]},
		...
		// Stage 3
		{
			...
			{
			    from: "cuttings_from_mapserver",
			    localField: "Lib Num",
			    foreignField: "properties.chlibno",
			    as: "mapserver_data"
			}
		},
		...
		// Stage 4
		{
			$project: {
			    ...
			    "sb_parent_id" : 1,
			    "intervals": 1,
			    "crcwc_url": {$concat: ["https://my.usgs.gov/crcwc/cutting/report/", {$toString: {"$arrayElemAt": ["$mapserver_data.id", 0]}}]}
			}
		},
